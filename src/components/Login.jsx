  ...
  return (
    <div className="shadow bg-white rounded-xl font-inter tracking-tighter flex flex-col items-center gap-5 my-auto p-10 max-w-4xl">
      ...
      <div className="form flex flex-col gap w-80">
        ...
        <button className="rounded-xl px-5 py-2 bg-black text-white my-5 hover:scale-[.99]" onClick={()=>setIsClicked(true)}>Sign in</button>
      </div>
      <p className="font-light">Don't have an account? <span className="font-semibold cursor-pointer" onClick={() =>setLoginPage(false)}>Sign up</span> </p>
    </div>
  );
};
