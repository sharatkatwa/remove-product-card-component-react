...
const App = () => {
  ...
  return (
    <div className=" w-full min-h-screen flex flex-col items-center bg-slate-50">
      {isClicked ? (
        <>
          <h1 className="font-bebas text-4xl py-10">Our Products</h1>
          <div className="flex gap-8 flex-wrap justify-center ">
            {productsArr.length !== 0 ? (
              productsArr.map((elem) => <ProductCard details={elem} removeProduct={removeProduct} key={elem.id} />)
            ) : (
              <h3 className="text-2xl text-gray-500 weight-light">No products available</h3>
            )}
          </div>
        </>
      ) : loginPage ? (
        <Login setLoginPage={setLoginPage} setIsClicked={setIsClicked} />
      ) : (
